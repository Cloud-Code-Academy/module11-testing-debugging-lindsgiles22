...
trigger LeadTrigger on Lead(before insert, before update, after insert, after update) {
	switch on Trigger.operationType {
		...
		when AFTER_UPDATE {
			List<Lead> leadsWithEmailChange = LeadTriggerHandler.getLeadsWithEmailChange(Trigger.new, Trigger.old);

			if (!leadsWithEmailChange.isEmpty()) {
			LeadTriggerHandler.handleLeadAutoConvert(leadsWithEmailChange);
		}
	}
}
}